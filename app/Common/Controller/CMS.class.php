...
class CMS extends Controller {
    ...
    protected function getModelObject($model) {
        if (is_string($model) && strpos($model, '/') == false) {
            ...
            if (strpos($model, '/') && is_string($model)) {
                $model = D($model);
            } else {
                if (is_object($model)) {
                    return $model;
    ...
    protected function validate(BaseValidator $validator, $data = null) {
        if (empty($data)) {
            $data = I('');
        }
        if ($validator instanceof BaseValidator) {
            $v = $validator->makeValidator();
        } else {
            $v = $validator;
        }
        ...
        try {
            ...
            $error_msg = $exception->getMainMessage();
        }

        if ($error_msg) {
            if (IS_AJAX) {
                $this->ajaxReturn(self::createReturn(false, null, $error_msg));
            } else {
